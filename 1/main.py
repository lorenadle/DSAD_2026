...
from functii import nan_replace, standardizare, salvare_ndarray, calcul_corelatii_covariante
...
corelatii_covariante = calcul_corelatii_covariante(x,tabel_date["Regiunea"].values)
for v in corelatii_covariante:
    salvare_ndarray(
        corelatii_covariante[v][0],
        variabile_numerice,
        variabile_numerice,
        "data_out/R_"+v+".csv"
    )
    salvare_ndarray(
        corelatii_covariante[v][1],
        variabile_numerice,
        variabile_numerice,
        "data_out/V_"+v+".csv"
    )
